...
def train(args, model, optimizer, scheduler=None, model_name='model'):
    ...
    for epoch in range(args.epochs):
        for batch_idx, (data, target, wgt) in enumerate(train_loader):
            ...
            gamma = 2.0
            alpha = 0.25
            focal_loss = -alpha * (1 - sigmoid_output)**gamma * target * torch.log(sigmoid_output) - \
                  (1 - alpha) * sigmoid_output**gamma * (1 - target) * torch.log(1 - sigmoid_output)
            weighted_loss = wgt * focal_loss
